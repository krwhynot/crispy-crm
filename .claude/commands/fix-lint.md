...
allowed-tools: Bash(npm run lint:apply, npm run prettier:apply, npm run validate:semantic-colors)
---

# Lint & Format Fixer

**Process:**
1. Run `npm run lint:apply` (ESLint auto-fix)
2. Run `npm run prettier:apply` (Prettier)
3. Run `npm run validate:semantic-colors` (semantic color validation)
...
|----------|--------|--------|
| ESLint auto-fixes | Complete | Check `npm run lint:apply` output |
| Prettier formatting | Complete | Check `npm run prettier:apply` output |
| Semantic colors | Varies | Check `npm run validate:semantic-colors` output for violations |
...
npm run lint && npx tsc --noEmit && npm run validate:semantic-colors

# Or individually:
npm run lint              # ESLint validation
npm run typecheck         # TypeScript validation
npm run validate:semantic-colors  # Semantic color validation
```
