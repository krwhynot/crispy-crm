...
#### [H7] Wrapper Composition Order Violation

**Files Affected:**
- `src/atomic-crm/providers/supabase/handlers/contactsHandler.ts:23`

**Risk:** Incorrect wrapper order breaks safety guarantees:
- Validation inside withErrorLogging means validation errors aren't logged
- LifecycleCallbacks inside withErrorLogging means callback errors aren't logged
- Errors can escape without being captured

**Correct Order (inside to outside):**
1. withValidation (innermost - validates first)
2. withLifecycleCallbacks (strips computed fields)
3. withErrorLogging (outermost - catches all errors)

```typescript
// WRONG: Validation bypasses error logging
return withValidation(
  withErrorLogging(baseProvider)
);

// CORRECT: Error logging wraps everything
export function createContactsHandler(baseProvider: DataProvider): DataProvider {
  return withErrorLogging(              // 3. Outermost - catches all errors
    withLifecycleCallbacks(             // 2. Strips computed fields
      withValidation(baseProvider),     // 1. Validates first
      [contactsCallbacks]
    )
  );
}
...
#### [M5] Business Logic in Handlers

**This is a manual check - requires reading handler files.**

**Look for:**
- Loops over arrays (>10 lines)
...
**Risk:** Handlers are meant to be thin plumbing that maps React Admin params to Supabase params. Business logic should live in:
- `src/services/` for complex operations
- `callbacks/` for lifecycle hooks
- `extensions/` for cross-cutting concerns

**Fix:** Extract business logic to appropriate service layer.

```typescript
// WRONG: Business logic in handler
export function createOpportunitiesHandler(baseProvider: DataProvider) {
  return {
    ...baseProvider,
    delete: async (resource, params) => {
      ...
      if (opportunity.activities.length > 0) {
        for (const activity of opportunity.activities) {
          await supabase.from('activities').update({ deleted_at: new Date() });
        }
      }
...
export function createOpportunitiesHandler(baseProvider: DataProvider) {
  return withErrorLogging(
    withLifecycleCallbacks(
      ...
      [opportunitiesCallbacks]  // Callbacks handle cascade delete via service
    )
  );
}

// In src/services/OpportunitiesService.ts
export async function archiveOpportunityWithActivities(opportunityId: string) {
  // Complex business logic lives here
  await supabase.rpc('archive_opportunity_cascade', { opp_id: opportunityId });
}
```
