...
const key = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFhcW5hbmRkY3F2Zml3aHNobmRsIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1ODU4MjE4NSwiZXhwIjoyMDc0MTU4MTg1fQ.8UTCpBUhDTFoNw9hMD43Tok3t3gYJ7cWEwwDoeB_nP4';
...
try {
  ...
  const principalId = principal?.id || (await supabase
    .from('organizations')
    .select('id')
  ...
  const oppId = opp?.id || (await supabase
    .from('opportunities')
    .select('id')
    .eq('name', 'Demo Active Deal')
    .single()).data.id;
