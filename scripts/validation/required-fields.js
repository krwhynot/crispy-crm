...
export class RequiredFieldsValidator {
  ...
  async validateCompanyRequiredFields() {
    ...
    if (companiesWithoutNames?.length > 0) {
      this.violations.push({
        ...
        samples: companiesWithoutNames.slice(0, 5).map(c =>
    ...
    const validSectors = ['Technology', 'Healthcare', 'Finance', 'Manufacturing', 'Retail', 'Other'];

    const { data: companiesWithInvalidSectors, error: sectorError } = await this.supabase
      .from('companies')
      .select('id, name, sector')
      .or(`sector.is.null,sector.eq.,sector.not.in.(${validSectors.map(s => `"${s}"`).join(',')})`);
    ...
    if (companiesWithInvalidSectors?.length > 0) {
      this.warnings.push({
        ...
        samples: companiesWithInvalidSectors.slice(0, 5).map(c =>
    ...
    if (companiesWithoutTimestamp?.length > 0) {
      this.warnings.push({
        ...
        samples: companiesWithoutTimestamp.slice(0, 5).map(c =>
  ...
  async validateContactRequiredFields() {
    ...
    if (contactsWithoutNames?.length > 0) {
      this.violations.push({
        ...
        samples: contactsWithoutNames.slice(0, 5).map(c =>
    ...
    const contactsWithoutContactInfo = contacts?.filter(contact => {
    ...
    if (contactsWithoutContactInfo.length > 0) {
      this.warnings.push({
        ...
        samples: contactsWithoutContactInfo.slice(0, 5).map(c =>
    ...
    if (contactsWithoutCompany?.length > 0) {
      this.violations.push({
        ...
        samples: contactsWithoutCompany.slice(0, 5).map(c =>
          `Contact "${c.first_name} ${c.last_name}" (ID: ${c.id})`
        ),
        fixable: true
      });
  ...
  async validateDealRequiredFields() {
    ...
    if (dealsWithoutNames?.length > 0) {
      this.violations.push({
        ...
        samples: dealsWithoutNames.slice(0, 5).map(d =>
          `Deal ID: ${d.id} (Company: ${d.company_id})`
        ),
        fixable: true
      });
    }
    ...
    if (dealsWithoutCompany?.length > 0) {
      this.violations.push({
        ...
        samples: dealsWithoutCompany.slice(0, 5).map(d =>
          `Deal "${d.name}" (ID: ${d.id})`
        ),
        fixable: true
      });
    ...
    if (dealsWithInvalidStages?.length > 0) {
      this.warnings.push({
        ...
        samples: dealsWithInvalidStages.slice(0, 5).map(d =>
          `Deal "${d.name}" (ID: ${d.id}) has stage: ${d.stage}`
        ),
        fixable: true
      });
    }
    ...
    if (dealsWithoutRevenue?.length > 0) {
      this.warnings.push({
        ...
        samples: dealsWithoutRevenue.slice(0, 5).map(d =>
  ...
  async validateContactNoteRequiredFields() {
    ...
    if (notesWithoutText?.length > 0) {
      this.violations.push({
        ...
        samples: notesWithoutText.slice(0, 5).map(n =>
          `Note ID: ${n.id} for Contact: ${n.contact_id}`
        ),
        fixable: true
      });
    }
  ...
  async validateDealNoteRequiredFields() {
    ...
    if (notesWithoutText?.length > 0) {
      this.violations.push({
        ...
        count: notesWithoutText.length,
        samples: notesWithoutText.slice(0, 5).map(n =>
          `Note ID: ${n.id} for Deal: ${n.deal_id}`
        ),
        fixable: true
      });
    }
  ...
  async validateTaskRequiredFields() {
    ...
    if (tasksWithoutType?.length > 0) {
      this.violations.push({
        ...
        samples: tasksWithoutType.slice(0, 5).map(t =>
    ...
    if (unassignedTasks?.length > 0) {
      this.warnings.push({
        ...
        samples: unassignedTasks.slice(0, 5).map(t =>
          `Task "${t.type}" (ID: ${t.id})`
        ),
        fixable: true
      });
  ...
  async validateTagRequiredFields() {
    ...
    if (tagsWithoutName?.length > 0) {
      this.violations.push({
        ...
        samples: tagsWithoutName.slice(0, 5).map(t =>
          `Tag ID: ${t.id} (${t.entity_type}:${t.entity_id})`
        ),
        fixable: true
      });
    }
  ...
  async validateMigrationSpecificRequirements() {
    ...
    const { data: companiesWithoutContacts, error } = await this.supabase.rpc('find_companies_without_contacts', {});
    ...
    if (companiesWithoutContacts?.length > 0) {
      this.warnings.push({
        ...
        samples: companiesWithoutContacts.slice(0, 5).map(c =>
          `Company "${c.name}" (ID: ${c.id})`
        ),
        fixable: true
      });
    ...
    const dealsWithoutContacts = deals?.filter(deal =>
    ...
    if (dealsWithoutContacts.length > 0) {
      this.warnings.push({
        ...
        samples: dealsWithoutContacts.slice(0, 5).map(d =>
          `Deal "${d.name}" (ID: ${d.id})`
        ),
        fixable: true
      });
    ...
    const { data: inconsistentDeals, error: consistencyError } = await this.supabase.rpc('find_deal_contact_company_mismatches', {});
    ...
    if (inconsistentDeals?.length > 0) {
      this.warnings.push({
        ...
        samples: inconsistentDeals.slice(0, 5).map(d =>
