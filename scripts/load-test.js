...
const ENDPOINTS = [
  ...
  { name: 'Get Contact with Orgs', weight: 10, fn: getContactWithOrganizations },
...
async function simulateUser(userId) {
  ...
  while (Date.now() - metrics.startTime < LOAD_TEST_CONFIG.testDuration * 1000) {
    ...
    try {
      ...
      metrics.requestsByEndpoint[endpoint.name] = (metrics.requestsByEndpoint[endpoint.name] || 0) + 1;
