...
const supabaseKey = process.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFhcW5hbmRkY3F2Zml3aHNobmRsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTg1ODIxODUsImV4cCI6MjA3NDE1ODE4NX0.wJi2sGLrvrI5OQUujTByVWjdyCT7Prjlpsx9LC_CUzU';
...
async function debugTaskRLS() {
  ...
  const { data: currentSalesIdCheck, error: currentSalesIdError } = await supabase
    .rpc('current_sales_id');
  ...
  const { data: isManagerOrAdmin, error: isManagerError } = await supabase
