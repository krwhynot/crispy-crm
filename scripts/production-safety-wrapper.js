    ...
    try {
      ...
      for (const entry of progress) {
        if (entry.status === 'in_progress') {
          const percentComplete = entry.total_rows > 0
      ...
      await this.logger.log('WARNING', 'Failed to check progress', { error: error.message });
    }
  }
...
class SafetyValidator {
  ...
  async confirmDowntime() {
    ...
    return new Promise((resolve) => {
      ...
      rl.question('\nHave you confirmed a ' + CONFIG.minimumDowntimeHours +
                  '-hour maintenance window? (type "YES" to confirm): ', (answer) => {
        rl.close();
        resolve(answer === 'YES');
      });
    });
