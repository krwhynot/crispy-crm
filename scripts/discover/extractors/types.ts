import type { InterfaceDeclaration, TypeAliasDeclaration, PropertySignature } from "ts-morph";
 ...
 */
const SOURCE_GLOBS = ["src/atomic-crm/types.ts", "src/atomic-crm/**/types.ts", "src/types/**/*.ts"];
 ...
 */
function extractGenerics(decl: InterfaceDeclaration | TypeAliasDeclaration): string[] | undefined {
 ...
 */
function extractFromInterface(decl: InterfaceDeclaration, filePath: string): TypeInfo {
 ...
 */
function extractFromTypeAlias(decl: TypeAliasDeclaration, filePath: string): TypeInfo {
  const name = decl.getName();
 ...
 * @param onlyChunks - Optional set of chunk keys to process (incremental mode)
 */
export async function extractTypes(onlyChunks?: Set<string>): Promise<void> {
  ...
  if (onlyChunks) {
    filesToProcess = sourceFiles.filter((sf) => {
      ...
      return onlyChunks.has(chunkKey);
    });
    console.log(
  ...
  for (const sourceFile of filesToProcess) {
    ...
    if (shouldExcludeFile(filePath)) {
      console.log(`  ⏭️  Skipping excluded file: ${path.basename(filePath)}`);
      continue;
    }
    ...
    const matchesGlob = relativePath.includes("types.ts") || relativePath.startsWith("src/types/");
    if (!matchesGlob) {
      continue;
    }
  ...
  // Use sourceFiles (not filesToProcess) to ensure full mapping is available
