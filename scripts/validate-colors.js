...
async function validateContrast() {
  ...
  // Define test cases
  const tagColorPairs = [
    'warm', 'green', 'teal', 'blue', 'purple',
    'yellow', 'gray', 'pink'
  ];

  const semanticTests = [
    { bg: 'primary', fg: 'primary-foreground', minRatio: 4.5, description: 'Primary button' },
    { bg: 'secondary', fg: 'secondary-foreground', minRatio: 4.5, description: 'Secondary button' },
    { bg: 'destructive', fg: 'destructive-foreground', minRatio: 4.5, description: 'Destructive button' },
    { bg: 'accent', fg: 'accent-foreground', minRatio: 4.5, description: 'Accent elements' },
    { bg: 'muted', fg: 'muted-foreground', minRatio: 4.5, description: 'Muted elements' },
    { bg: 'card', fg: 'card-foreground', minRatio: 4.5, description: 'Card content' },
    { bg: 'popover', fg: 'popover-foreground', minRatio: 4.5, description: 'Popover content' },
  ];

  const focusTests = [
    { bg: 'background', fg: 'ring', minRatio: 3.0, description: 'Focus ring on background' },
    { bg: 'card', fg: 'ring', minRatio: 3.0, description: 'Focus ring on cards' },
  ];
  ...
  for (const mode of ['light', 'dark']) {
    ...
    for (const test of semanticTests) {
      ...
      if (passes) {
        ...
        results.summary.failed++;
        console.log(`❌ ${test.description}: ${ratio.toFixed(2)}:1 (min ${test.minRatio}:1) - FAILED`);
      }
    }
