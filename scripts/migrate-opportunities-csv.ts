...
async function migrateOpportunities(): Promise<void> {
  try {
    ...
    for (const [index, row] of validRows.entries()) {
      ...
      if (!customerOrgId) {
        console.log(
          `   ⚠️  Row ${index + 16}: Skipping "${opportunityName}" - customer org "${customerOrgName}" not found`
        );
        skipped++;
        continue;
      }
      ...
      if (error) {
        console.error(
          `   ❌ Row ${index + 16}: Failed to import "${opportunityName}":`,
          error.message
        );
      ...
      if (opportunity) {
        ...
        if (backfillError) {
          console.warn(
            `   ⚠️  Row ${index + 16}: Contact backfill failed for "${opportunityName}":`,
