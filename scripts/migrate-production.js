...
class CRMMigration {
  ...
  async executeSQL(sqlContent, phaseName) {
    try {
      ...
      await this.logger.error(`SQL execution failed for ${phaseName}`, { error: error.message });
      throw error;
    }
  }

  async executePhase(phase) {
    await this.logger.info(`Starting phase: ${phase.name}`, { phaseId: phase.id });
    ...
    try {
      ...
      await this.logger.info(`Phase completed: ${phase.name}`, { phaseId: phase.id });
