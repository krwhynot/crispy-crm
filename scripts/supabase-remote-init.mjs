...
async function linkProject({ projectRef, databasePassword }) {
  await execa(
    ...
    return sleep(1000).then(() => linkProject({ projectRef, databasePassword }));
  });
}
...
async function setupDatabase({ databasePassword }) {
  await execa(
    "npx",
    ["supabase", "db", "push", "--linked", "--include-roles", "--password", databasePassword],
    {
      stdio: "inherit",
    }
  );
...
async function fetchApiKeys({ projectRef }) {
  ...
  try {
    const { stdout, exitCode } = await execa(
      "npx",
      ["supabase", "projects", "api-keys", "--output", "json", "--project-ref", projectRef],
      {
        stdout: "pipe",
        stderr: "ignore",
      }
