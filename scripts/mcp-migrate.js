...
class MCPMigrationEngine {
  ...
  async initialize(options = {}) {
    ...
    this.log('MCP Migration Engine initialized', 'info', { projectId: this.projectId, dryRun: this.dryRun });
  }
  ...
  async loadState() {
    try {
      if (existsSync(STATE_FILE)) {
        ...
        this.log(`Loaded migration state: ${this.state.status}, applied: ${this.state.appliedMigrations.join(', ')}`);
      }
    } catch (error) {
  ...
  async validateMigration(filename, content) {
    const validationRules = [
      ...
      },
      {
        ...
        test: (content) => content.includes('rollback') || content.includes('ROLLBACK') || content.includes('-- ROLLBACK:'),
        severity: 'info'
      }
    ];
    ...
    if (issues.length > 0) {
      ...
      if (hasErrors && !this.dryRun) {
        const proceed = await this.prompt('Migration has validation errors. Continue anyway? (yes/no): ');
