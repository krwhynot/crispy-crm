...
async function validateExports() {
  ...
  try {
    ...
    const hasExport = content.includes('export') && content.includes('dataProvider');

    results.push({
      test: 'Provider exported correctly',
      status: hasExport ? 'PASS' : 'FAIL',
      message: hasExport ? undefined : 'dataProvider not exported from index.ts'
    });
  } catch (error) {
    results.push({
      test: 'Provider exported correctly',
...
async function validateTests() {
  try {
    execSync('npm run test -- --run --reporter=silent', { encoding: 'utf-8', stdio: 'pipe' });
    results.push({
      test: 'Unit tests',
      status: 'PASS'
    });
...
async function runValidation() {
  ...
  for (const result of results) {
    ...
    console.log(`${icon} ${color(result.test.padEnd(30))} ${result.message || ''}`);
