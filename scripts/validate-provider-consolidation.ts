...
async function validateSingleProviderArchitecture() {
  ...
  addResult("Architecture", "Old dataProvider.ts removed", oldExists ? "FAIL" : "PASS",
  ...
  addResult("Architecture", "Unified provider exists", unifiedExists ? "PASS" : "FAIL",
  ...
  const hasCorrectExport = indexContent.includes("unifiedDataProvider") &&
                          (indexContent.includes("export") && indexContent.includes("dataProvider"));
  addResult("Architecture", "Provider exports updated", hasCorrectExport ? "PASS" : "FAIL",
  ...
  try {
    ...
    addResult("Architecture", "No old provider imports", hasOldImports ? "WARN" : "PASS",
...
async function validateAllFeaturesWorking() {
  ...
  addResult("CRUD", "Basic CRUD operations", missingCrud.length === 0 ? "PASS" : "FAIL",
    missingCrud.length > 0 ? `Missing: ${missingCrud.join(", ")}` : undefined);

  // 2.2 File Upload Functionality
  const hasFileUpload = unifiedContent.includes("uploadToBucket") ||
                       unifiedContent.includes("upload") && unifiedContent.includes("Storage");
  addResult("CRUD", "File upload functionality", hasFileUpload ? "PASS" : "WARN",
  ...
  addResult("CRUD", "Junction table operations", missingJunctions.length === 0 ? "PASS" : "WARN",
    missingJunctions.length > 0 ? `Missing junction methods: ${missingJunctions.join(", ")}` : undefined);

  // 2.4 Search and Filtering
  const hasSearch = unifiedContent.includes("fts") || unifiedContent.includes("search");
  addResult("CRUD", "Search functionality", hasSearch ? "PASS" : "WARN",
    hasSearch ? undefined : "Full-text search functionality not found");

  const hasSoftDelete = unifiedContent.includes("deleted_at") || unifiedContent.includes("soft delete");
  addResult("CRUD", "Soft delete filtering", hasSoftDelete ? "PASS" : "WARN",
    hasSoftDelete ? undefined : "Soft delete filtering not found");

  // 2.5 Custom Business Operations
  const customMethods = ["salesCreate", "salesUpdate", "updatePassword", "unarchiveOpportunity", "getActivityLog"];
  const missingCustom = customMethods.filter(method => !unifiedContent.includes(method));
  addResult("CRUD", "Custom business methods", missingCustom.length === 0 ? "PASS" : "WARN",
...
async function validateCleanArchitecture() {
  ...
  const hasValidationRegistry = unifiedContent.includes("validationRegistry") ||
                               unifiedContent.includes("ValidationConfig");
  addResult("Architecture", "Validation registry", hasValidationRegistry ? "PASS" : "WARN",
    hasValidationRegistry ? undefined : "Validation registry pattern not found");

  // 3.2 Transformation Registry
  const hasTransformRegistry = unifiedContent.includes("transformerRegistry") ||
                              unifiedContent.includes("TransformerConfig") ||
                              unifiedContent.includes("transform");
  addResult("Architecture", "Transformation registry", hasTransformRegistry ? "PASS" : "WARN",
  ...
  addResult("Architecture", "Service layer exists", servicesExist ? "PASS" : "WARN",
    servicesExist ? undefined : "Service layer directory not found");

  if (servicesExist) {
    const serviceFiles = ["sales.service.ts", "opportunities.service.ts", "activities.service.ts", "junctions.service.ts"];
    for (const serviceFile of serviceFiles) {
      const servicePath = join(servicesPath, serviceFile);
      const exists = await fileExists(servicePath);
      addResult("Architecture", `${serviceFile} service`, exists ? "PASS" : "WARN",
  ...
  if (utilsExist) {
    ...
    addResult("Architecture", "Storage utilities extracted", storageUtilsExist ? "PASS" : "WARN",
  ...
  addResult("Architecture", "No mixed concerns", hasMixedConcerns ? "WARN" : "PASS",
...
async function validateTestsPassing() {
  ...
  const dataProviderTestPath = join(process.cwd(), "src/atomic-crm/providers/supabase/unifiedDataProvider.test.ts");
  const providerTestExists = await fileExists(dataProviderTestPath);
  addResult("Tests", "Provider unit tests", providerTestExists ? "PASS" : "WARN",
  ...
  const junctionTestPath = join(process.cwd(), "tests/performance/junction-table-performance.spec.ts");
  const junctionTestExists = await fileExists(junctionTestPath);
  addResult("Tests", "Junction performance tests", junctionTestExists ? "PASS" : "WARN",
...
async function validateConstitutionCompliance() {
  ...
  const oldProviderPath = join(process.cwd(), "src/atomic-crm/providers/supabase/dataProvider.ts");
  const oldExists = await fileExists(oldProviderPath);
  addResult("Constitution", "Single data provider", oldExists ? "FAIL" : "PASS",
    oldExists ? "Multiple data providers still exist" : undefined);

  // 5.2 Principle #3: Zod schemas at API boundary only
  const unifiedPath = join(process.cwd(), "src/atomic-crm/providers/supabase/unifiedDataProvider.ts");
  const unifiedContent = await readFileContent(unifiedPath);
  const hasValidationBoundary = unifiedContent.includes("validationRegistry") ||
                               unifiedContent.includes("validate");
  addResult("Constitution", "Zod at API boundary", hasValidationBoundary ? "PASS" : "WARN",
  ...
  addResult("Constitution", "Single responsibility", hasSingleResponsibility ? "PASS" : "WARN",
    hasSingleResponsibility ? undefined : "Mixed concerns detected (lifecycle callbacks)");

  // 5.4 Principle #9: No backwards compatibility
  const hasNoBackwardCompat = !unifiedContent.includes("deprecated") &&
                             !unifiedContent.includes("backward") &&
                             !unifiedContent.includes("legacy");
  addResult("Constitution", "No backward compatibility", hasNoBackwardCompat ? "PASS" : "WARN",
...
async function validateCRUDOperations() {
  ...
  if (validationExists) {
    for (const resource of resources) {
      ...
      addResult("CRUD", `${resource} validation`, exists ? "PASS" : "WARN",
  ...
  addResult("CRUD", "Resource mapping", resourcesExist ? "PASS" : "WARN",
...
async function runValidation() {
  ...
  if (criticalFailures === 0) {
    ...
    if (totalWarn > 0) {
      console.log(chalk.yellow(`⚠️  ${totalWarn} warnings should be addressed but don't block consolidation`));
    }
