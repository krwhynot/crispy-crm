...
rl.on('close', () => {
  ...
  if (rules['@typescript-eslint/no-explicit-any'] > 0) {
    console.log(`  • Priority: Fix ${rules['@typescript-eslint/no-explicit-any']} explicit 'any' usages`);
  ...
  if (rules['@typescript-eslint/no-unsafe-assignment'] > 0) {
    console.log(`  • Review ${rules['@typescript-eslint/no-unsafe-assignment']} unsafe assignments for potential type violations`);
  }

  if (rules['@typescript-eslint/no-unsafe-member-access'] > 0) {
    console.log(`  • ${rules['@typescript-eslint/no-unsafe-member-access']} unsafe member accesses could cause runtime errors`);
  }
