...
async function checkDatabaseObjects() {
  ...
  try {
    ...
    for (const table of junctionTables) {
      ...
      if (!error || error.code === 'PGRST116') { // Empty table is OK
...
function checkCriticalFiles() {
  ...
  const criticalFiles = [
    // Migration scripts
    { path: 'scripts/migration-execute.js', desc: 'Migration execution script' },
    { path: 'scripts/migration-rollback.js', desc: 'Rollback script' },
    { path: 'scripts/migration-monitor.js', desc: 'Monitoring script' },
    { path: 'scripts/migration-state-tracker.js', desc: 'State tracking script' },
    { path: 'scripts/migration-cleanup.js', desc: 'Cleanup script' },
    { path: 'scripts/migration-backup.js', desc: 'Backup script' },
    { path: 'scripts/post-migration-validation.js', desc: 'Post-migration validation' },
    { path: 'scripts/cache-invalidation.js', desc: 'Cache invalidation' },

    // Validation scripts
    { path: 'scripts/validation/data-quality.js', desc: 'Data quality validator' },
    { path: 'scripts/validation/referential-integrity.js', desc: 'Referential integrity' },
    { path: 'scripts/validation/go-no-go.js', desc: 'Go/No-Go decision' },

    // SQL files
    { path: 'docs/merged/migrations/stage1/001_phase_1_1_foundation_setup.sql', desc: 'Foundation setup SQL' },
    { path: 'docs/merged/migrations/stage1/002_phase_1_2_contact_organization_relationships.sql', desc: 'Relationships SQL' },

    // Opportunity components
    { path: 'src/atomic-crm/opportunities/index.ts', desc: 'Opportunities index' },
    { path: 'src/atomic-crm/opportunities/OpportunityList.tsx', desc: 'Opportunity list' },
    { path: 'src/atomic-crm/opportunities/OpportunityShow.tsx', desc: 'Opportunity show' },
    { path: 'src/atomic-crm/opportunities/OpportunityCreate.tsx', desc: 'Opportunity create' },

    // Backward compatibility
    { path: 'src/atomic-crm/providers/commons/backwardCompatibility.ts', desc: 'Backward compatibility' }
  ];
...
function checkUITextMigration() {
  ...
  const componentsToCheck = fs.readdirSync(opportunityDir)
...
function checkBackwardCompatibility() {
  ...
  const bcPath = path.join(__dirname, '..', 'src/atomic-crm/providers/commons/backwardCompatibility.ts');
...
function checkTestCoverage() {
  ...
  const testFiles = [
    // Unit tests
    { path: 'src/atomic-crm/opportunities/OpportunityList.spec.tsx', desc: 'OpportunityList tests' },
    { path: 'src/atomic-crm/opportunities/OpportunityShow.spec.tsx', desc: 'OpportunityShow tests' },
    { path: 'src/atomic-crm/BackwardCompatibility.spec.ts', desc: 'Backward compatibility tests' },

    // Migration tests
    { path: 'tests/migration/dry-run.spec.ts', desc: 'Dry-run tests' },
    { path: 'tests/migration/rollback.spec.ts', desc: 'Rollback tests' },
    { path: 'tests/migration/data-integrity.spec.ts', desc: 'Data integrity tests' },

    // Performance tests
    { path: 'tests/performance/opportunity-queries.spec.ts', desc: 'Performance tests' }
  ];
...
function checkProductionSafety() {
  ...
  const safetyFeatures = [
    ...
    { pattern: 'SET statement_timeout', desc: 'Statement timeout configuration' },
...
function generateReport() {
  ...
  if (isReady) {
    console.log(`${colors.green}${colors.bright}✓ MIGRATION READY - System is prepared for production migration${colors.reset}`);
    console.log(`${colors.green}Note: Address warnings before proceeding if possible${colors.reset}`);
  } else {
    console.log(`${colors.red}${colors.bright}✗ NOT READY - Critical issues must be resolved${colors.reset}`);
    console.log(`${colors.red}DO NOT PROCEED with migration until all failures are fixed${colors.reset}`);
  }
 ...
 */
async function main() {
  console.log(`${colors.cyan}${colors.bright}CRM Migration - Final Verification (Task 4.7)${colors.reset}`);
