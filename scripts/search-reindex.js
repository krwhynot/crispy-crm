...
const specificTables = tablesArg ? tablesArg.split('=')[1].split(',').map(t => t.trim()) : null;
...
async function rebuildBTreeIndexes() {
  ...
  const targetTables = specificTables || ['opportunities', 'contacts', 'companies', 'contact_organizations', 'opportunity_participants'];
  const btreeIndexes = indexes.filter(idx =>
...
async function rebuildGinIndexes() {
  ...
  const ginIndexes = [
    ...
    },
    {
      ...
      createSql: 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_tags_gin ON contacts USING gin(tags);'
    },
    {
      table: 'opportunities',
      column: 'contact_ids',
      indexName: 'idx_opportunities_contact_ids_gin',
      createSql: 'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_opportunities_contact_ids_gin ON opportunities USING gin(contact_ids);'
    }
  ];
...
async function updateTableStatistics() {
  ...
  const tables = specificTables || [
    ...
    'contact_organizations', 'opportunity_participants',
    'contactNotes', 'dealNotes', 'opportunityNotes', 'tasks'
  ];
