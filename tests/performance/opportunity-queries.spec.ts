...
describe('Opportunity Queries Performance Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const { data: orgs, error: orgError } = await supabase
      .from('companies')
      .insert([
        { name: 'Test Customer Org', organization_type: 'customer', sector: 'Technology' },
        { name: 'Test Principal Org', organization_type: 'principal', sector: 'Manufacturing' },
        { name: 'Test Distributor Org', organization_type: 'distributor', sector: 'Retail' }
      ])
      .select('id');
    ...
    for (let i = 0; i < 10000; i++) {
      opportunities.push({
        ...
        probability: faker.number.float({ min: 0, max: 100, fractionDigits: 0 }),
  ...
  afterAll(async () => {
    ...
    console.table(results.map(r => ({
      ...
      'vs Baseline': r.baselineComparison ? `${((r.executionTime / r.baselineComparison - 1) * 100).toFixed(1)}%` : 'N/A'
    })));
  });
  ...
  it('should execute aggregation query within performance threshold', async () => {
    ...
    const { data, error } = await supabase
      .rpc('get_opportunity_stats_by_stage');
  ...
  it('should handle concurrent queries efficiently', async () => {
    ...
    expect(avgTimePerQuery).toBeLessThan(PERFORMANCE_THRESHOLDS.simpleList * 1.5);
  });
});