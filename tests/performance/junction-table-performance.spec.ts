...
describe("Junction Table Performance Tests - Unified Provider", () => {
  ...
  beforeAll(async () => {
    ...
    const organizations = Array(100)
      .fill(null)
      .map((_, i) => ({
        name: `Junction Test Org ${i}`,
        organization_type: ["customer", "principal", "distributor", "partner"][
          i % 4
        ],
    ...
    console.log(
      `Created ${contactOrgRelationships.length} contact-organization relationships`,
    );

    // Create test opportunities
    const opportunities = Array(200)
      .fill(null)
      .map((_, i) => ({
        name: `Junction Test Opportunity ${i}`,
        customer_organization_id: testOrgIds[i % testOrgIds.length],
        principal_organization_id:
          i % 3 === 0 ? testOrgIds[(i + 1) % testOrgIds.length] : null,
        distributor_organization_id:
          i % 4 === 0 ? testOrgIds[(i + 2) % testOrgIds.length] : null,
      ...
      for (let j = 0; j < numParticipants; j++) {
        ...
        if (!usedOrgs.has(orgId)) {
          ...
          opportunityParticipants.push({
            ...
            is_primary: j === 0,
            commission_rate:
              j === 1 ? faker.number.float({ min: 0, max: 20 }) : null,
    ...
    for (let i = 0; i < opportunityParticipants.length; i += batchSize) {
      ...
      if (error)
        console.error("Error inserting opportunity participants batch:", error);
    }

    console.log(
      `Created ${opportunityParticipants.length} opportunity participants`,
    );

    // Create test activities
    const activities = Array(300)
      .fill(null)
      .map((_, i) => ({
        ...
        opportunity_id:
          i % 2 === 1
            ? testOpportunityIds[i % testOpportunityIds.length]
            : null,
    ...
    for (let i = 0; i < testActivityIds.length; i++) {
      ...
      for (let j = 0; j < numParticipants; j++) {
        interactionParticipants.push({
          ...
          contact_id:
            j % 2 === 0
              ? testContactIds[(i + j) % testContactIds.length]
              : null,
    ...
    for (let i = 0; i < interactionParticipants.length; i += batchSize) {
      ...
      if (error)
        console.error("Error inserting interaction participants batch:", error);
    }

    console.log(
      `Created ${interactionParticipants.length} interaction participants`,
    );
  });
  ...
  it("should efficiently join contacts with multiple organizations via unified provider", async () => {
    ...
    const totalRelationships =
      data?.reduce(
        (sum, contact) => sum + (contact.contact_organizations?.length || 0),
        0,
      ) || 0;
  ...
  it("should efficiently query opportunity participants via service methods", async () => {
    ...
    const totalParticipants =
      data?.reduce(
        (sum, opp) => sum + (opp.opportunity_participants?.length || 0),
        0,
      ) || 0;
    ...
    expect(executionTime).toBeLessThan(
      JUNCTION_THRESHOLDS.opportunityParticipantJoin,
    );
  });
  ...
  it("should efficiently query interaction participants via unified provider", async () => {
    ...
    const totalParticipants =
      data?.reduce(
        (sum, activity) =>
          sum + (activity.interaction_participants?.length || 0),
        0,
      ) || 0;
    ...
    results.push(result);
    expect(executionTime).toBeLessThan(
      JUNCTION_THRESHOLDS.interactionParticipantJoin,
    );
  });
