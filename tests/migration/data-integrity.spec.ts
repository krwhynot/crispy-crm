...
describe("Migration Data Integrity Tests", () => {
  describe("Record Count Preservation", () => {
    ...
    it("should account for new relationship records correctly", async () => {
      ...
      const relationshipValidation =
        await mockRelationshipValidator.validateNewRelationships();
      expect(relationshipValidation.allRelationshipsValid).toBe(true);
      expect(relationshipValidation.contactOrganizations.correctlyMapped).toBe(
        true,
      );
      expect(
        relationshipValidation.opportunityParticipants.correctlyMapped,
      ).toBe(true);
    });
  });
  ...
  describe("100-Sample Record Verification", () => {
    it("should verify opportunity data integrity", async () => {
      ...
      const sampleOpportunities =
        await mockSampleValidator.selectRandomOpportunities();
      expect(sampleOpportunities).toHaveLength(3); // Mock sample

      const verification =
        await mockSampleValidator.verifyOpportunityTransformation();
    ...
    it("should verify contact organization relationship creation", async () => {
      ...
      const verification =
        await mockContactValidator.verifyContactOrganizationMappings();
      expect(verification.mappingAccuracy).toBeGreaterThan(90.0);
      expect(verification.junctionRecordsCreated).toBe(
        verification.contactsWithCompany,
      );
    });
    ...
    it("should verify opportunity participant relationships", async () => {
      ...
      const sampleOpportunities =
        await mockParticipantValidator.selectRandomOpportunities();
      expect(sampleOpportunities).toHaveLength(3); // Mock sample

      const verification =
        await mockParticipantValidator.verifyParticipantMappings();
    ...
    it("should detect and handle orphaned records", async () => {
      ...
      if (orphanDetection.totalOrphans > 0) {
        const orphanResolution =
          await mockOrphanDetector.handleOrphanedRecords();
  ...
  describe("Data Type and Constraint Validation", () => {
    it("should validate enum value transformations", async () => {
      ...
      const stageValidation =
        await mockEnumValidator.validateStageEnumTransformation();
      ...
      const priorityValidation =
        await mockEnumValidator.validatePriorityEnumValues();
    ...
    it("should validate numeric constraints and ranges", async () => {
      ...
      const valueValidation =
        await mockNumericValidator.validateOpportunityValues();
      ...
      const probabilityValidation =
        await mockNumericValidator.validateProbabilityConstraints();
    ...
    it("should validate timestamp and date integrity", async () => {
      ...
      const timestampValidation =
        await mockDateValidator.validateTimestampPreservation();
  ...
  describe("Edge Case Data Handling", () => {
    it("should handle null values correctly", async () => {
      ...
      expect(nullValidation.junctionRecordsSkipped).toBe(
        nullValidation.nullCompanyIdsInContacts,
      );

      const optionalNullValidation =
        await mockNullHandler.validateOptionalFieldNulls();
    ...
    it("should handle special characters and Unicode correctly", async () => {
      ...
      const specialCharValidation =
        await mockSpecialCharHandler.validateSpecialCharacters();
      ...
      const jsonbValidation =
        await mockSpecialCharHandler.validateJsonbFields();
    ...
    it("should handle large text fields correctly", async () => {
      ...
      const textValidation =
        await mockLargeTextHandler.validateLargeTextFields();
      ...
      const encodingValidation =
        await mockLargeTextHandler.validateTextEncoding();
  ...
  describe("Business Rule Compliance", () => {
    it("should validate opportunity probability assignments", async () => {
      ...
      const probabilityValidation =
        await mockBusinessRuleValidator.validateProbabilityByStage();
      ...
      expect(
        probabilityValidation.stageBasedProbabilities["closed_won"]
          .avgProbability,
      ).toBe(100);
      expect(
        probabilityValidation.stageBasedProbabilities["closed_lost"]
          .avgProbability,
      ).toBe(0);
    });
    ...
    it("should validate primary contact assignments", async () => {
      ...
      const primaryContactValidation =
        await mockContactRuleValidator.validatePrimaryContactRules();
      expect(primaryContactValidation.primaryContactRulesValid).toBe(true);
      expect(primaryContactValidation.organizationsWithMultiplePrimaries).toBe(
        0,
      );
      expect(primaryContactValidation.compliancePercentage).toBeGreaterThan(
        90.0,
      );
    });
    ...
    it("should validate customer organization assignments", async () => {
      ...
      const customerValidation =
        await mockCustomerValidator.validateCustomerOrganizations();
