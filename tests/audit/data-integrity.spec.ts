...
describe('Data Integrity and Compliance Tests', () => {
  ...
  describe('Data Retention Policy Compliance', () => {
    it('should preserve all archived records during migration', async () => {
      ...
      for (let i = 0; i < archivedDeals.length; i++) {
        expect(archivedOpportunities[i].archived_at).toBe(archivedDeals[i].archived_at);
        expect(archivedOpportunities[i].created_at).toBe(archivedDeals[i].created_at);
        expect(archivedOpportunities[i].sales_id).toBe(archivedDeals[i].sales_id);
      }
    });
    ...
    it('should comply with data retention periods for different record types', async () => {
      ...
      const recordCounts = {
        opportunities: { total: 150, withinRetention: 150, outsideRetention: 0 },
      ...
      for (const [entity, counts] of Object.entries(recordCounts)) {
        if (counts.outsideRetention > 0 && retentionPolicies[entity].hasArchivedRecords) {
          // Records outside retention should be in archived state, not deleted
          expect(counts.total).toBe(counts.withinRetention + counts.outsideRetention);
        }
      }
    });
  });
  ...
  describe('No Data Loss Validation', () => {
    it('should verify zero data loss across all core entities', async () => {
      ...
      // Verify no data loss
      expect(dataLossValidation.postMigrationCounts.opportunities)
        .toBe(dataLossValidation.preMigrationCounts.deals);
      expect(dataLossValidation.postMigrationCounts.contacts)
        .toBe(dataLossValidation.preMigrationCounts.contacts);
      expect(dataLossValidation.postMigrationCounts.companies)
        .toBe(dataLossValidation.preMigrationCounts.companies);
      expect(dataLossValidation.postMigrationCounts.opportunityNotes)
        .toBe(dataLossValidation.preMigrationCounts.dealNotes);

      // Calculate total records
      const totalPre = Object.values(dataLossValidation.preMigrationCounts)
        .reduce((sum, count) => sum + count, 0);
      const totalPost = Object.values(dataLossValidation.postMigrationCounts)
        .reduce((sum, count) => sum + count, 0);
    ...
    it('should validate data integrity for complex relationships', async () => {
      ...
      expect(relationshipValidation.contactCompanyRelationships.postMigration.contactOrganizations)
        .toBeGreaterThanOrEqual(relationshipValidation.contactCompanyRelationships.preMigration);

      expect(relationshipValidation.dealContactRelationships.postMigration.allParticipantsLinked)
        .toBe(true);
    });
  ...
  describe('Rollback Data Restoration', () => {
    it('should verify backup tables contain complete data snapshots', async () => {
      ...
      expect(backupValidation.backupTables[0].recordCount)
        .toBe(backupValidation.originalCounts.deals);
      expect(backupValidation.backupTables[1].recordCount)
        .toBe(backupValidation.originalCounts.contacts);
      expect(backupValidation.backupTables[2].recordCount)
        .toBe(backupValidation.originalCounts.companies);
    });
    ...
    it('should validate rollback capability within 48-hour window', async () => {
      ...
      const elapsedMs = rollbackWindow.currentTime.getTime() - rollbackWindow.backupCreated.getTime();
    ...
    it('should verify rollback scripts can restore original schema', async () => {
      const rollbackScriptValidation = {
        ...
        rollbackSteps: [
          { step: 1, action: 'Restore deals table from backup', canExecute: true },
          { step: 2, action: 'Restore original contact.company_id', canExecute: true },
          { step: 3, action: 'Drop new junction tables', canExecute: true },
          { step: 4, action: 'Restore original views', canExecute: true },
          { step: 5, action: 'Restore original RLS policies', canExecute: true }
        ],
      ...
      expect(rollbackScriptValidation.rollbackSteps.every(s => s.canExecute)).toBe(true);
    ...
    it('should validate backup field preservation for safe rollback', async () => {
      ...
      expect(backupFieldValidation.contacts.withCompanyIdBackup)
        .toBe(backupFieldValidation.contacts.backupMatchesOriginal);

      expect(backupFieldValidation.opportunities.withOriginalDealData)
        .toBe(backupFieldValidation.opportunities.totalRecords);

      // Verify stage mapping can be reversed
      expect(Object.keys(backupFieldValidation.opportunities.stageMapping)).toHaveLength(5);
    });
  });
  ...
  describe('Compliance Verification', () => {
    it('should validate compliance with data governance requirements', async () => {
      ...
      expect(complianceChecks.dataQuality.referentialIntegrityMaintained).toBe(true);
        ...
        transformations: [
          {
            ...
            transformationRules: ['stage_mapping', 'status_derivation', 'field_renaming']
          },
          {
            ...
            transformationRules: ['relationship_extraction', 'primary_flag_setting']
          }
        ],
        auditTrailComplete: true,
  ...
  describe('Critical Data Validation', () => {
    it('should ensure financial data integrity is maintained', async () => {
      ...
      expect(financialDataValidation.postMigrationTotals.totalOpportunityValue)
        .toBe(financialDataValidation.preMigrationTotals.totalDealValue);

      expect(financialDataValidation.postMigrationTotals.averageOpportunitySize)
        .toBeCloseTo(financialDataValidation.preMigrationTotals.averageDealSize, 2);
