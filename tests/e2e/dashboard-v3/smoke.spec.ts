...
test.describe('Dashboard V3 - Smoke Test', () => {
  test('dashboard-v3 route is accessible and renders', async ({ authenticatedPage }) => {
    ...
    const errorBoundary = await authenticatedPage.locator('text=/error|failed|something went wrong/i').first().isVisible().catch(() => false);
    const dashboardHeader = await authenticatedPage.locator('h1').textContent().catch(() => '');
