...
test.describe('Dashboard V3 - Full Stack E2E', () => {
  ...
  test.describe('Panel Rendering and Loading States', () => {
    test('renders all three panels with correct headers', async ({ authenticatedPage }) => {
      ...
      await expect(
        authenticatedPage.locator('h3:has-text("Pipeline by Principal")')
      ).toBeVisible();

      // Tasks panel header
      await expect(
        authenticatedPage.locator('h3:has-text("My Tasks")')
      ).toBeVisible();

      // Quick Logger panel header
      await expect(
        authenticatedPage.locator('h2:has-text("Log Activity")')
      ).toBeVisible();
    });
    ...
    test('displays pipeline data from principal_pipeline_summary view', async ({ authenticatedPage }) => {
      ...
      for (const header of headers) {
        await expect(
          authenticatedPage.locator(`th:has-text("${header}")`)
        ).toBeVisible();
      }
      ...
      if (rowCount > 0) {
        ...
        const momentumCell = authenticatedPage.locator('td').filter({
          has: authenticatedPage.locator('.lucide-trending-up, .lucide-trending-down, .lucide-minus'),
        }).first();
  ...
  test.describe('Task Interactions', () => {
    test('completes task via checkbox and updates UI', async ({ authenticatedPage }) => {
      ...
      const taskStillVisible = await authenticatedPage
        .locator(`.interactive-card:has-text("${taskSubject}")`).first()
  ...
  test.describe('Activity Logging', () => {
    test('logs activity with required fields and closes form', async ({ authenticatedPage }) => {
      ...
      const activityTypeSelect = authenticatedPage.locator('select').filter({
      ...
      const orgSelect = authenticatedPage.locator('select').filter({
  ...
  test.describe('Resizable Panels and Persistence', () => {
    ...
    test('restores panel sizes from localStorage on page reload', async ({ authenticatedPage }) => {
      ...
      await expect(
        authenticatedPage.locator('h1:has-text("Principal Dashboard")')
      ).toBeVisible();
  ...
  test.describe('Error Handling', () => {
    test('displays error state when pipeline view fails', async ({ authenticatedPage }) => {
      ...
      const errorIndicator = authenticatedPage.locator('text=/failed to load|error|unable to fetch/i').first();
  ...
  test.describe('Accessibility', () => {
    test('all interactive elements meet WCAG 2.1 AA touch target size', async ({ authenticatedPage }) => {
      // Get all buttons and checkboxes
      const interactiveElements = await authenticatedPage.locator('button, input[type="checkbox"]').all();
