...
test.describe("Dashboard Quick Actions - Complete Task Workflow", () => {
  ...
  test.describe("Modal Opening and Step 1 (Log Activity)", () => {
    test("opens modal when task checkbox is clicked", async ({ authenticatedPage }) => {
      ...
      await expect(
        authenticatedPage.getByRole("heading", { name: /Complete Task:/i })
      ).toBeVisible();
    });
    ...
    test.skip("auto-detects activity type from task title", async ({ authenticatedPage }) => {
      ...
      const activityTypeCombobox = authenticatedPage.getByRole("combobox", {
  ...
  test.describe("Step 2 (Update Opportunity) - With Opportunity", () => {
    test("advances to Step 2 after logging activity", async ({ authenticatedPage }) => {
      ...
      await expect(
        authenticatedPage.getByRole("heading", { name: /Update Opportunity/i })
      ).toBeVisible();
    ...
    test("allows skipping opportunity update", async ({ authenticatedPage }) => {
      ...
      await expect(
        authenticatedPage.getByRole("heading", { name: /Task Completed!/i })
      ).toBeVisible();
    });
    ...
    test("shows stage transition indicator when stage is selected", async ({
      authenticatedPage,
    }) => {
      // Complete Step 1
      await authenticatedPage.getByRole("checkbox").first().click();
      ...
      await expect(
        authenticatedPage.getByRole("button", { name: /Update & Close/i })
      ).toBeVisible();
    });
