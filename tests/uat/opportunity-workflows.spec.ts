...
describe("UAT: Opportunity Workflows", () => {
  ...
  describe("Create Opportunity with All New Fields", () => {
    test("should validate new opportunity fields are available", () => {
      ...
      expect([
        ...
        "Closed Won",
        "Closed Lost",
      ]).toContain(newOpportunity.lifecycle_stage);
    });
  ...
  describe("Multi-Organization Contact Support", () => {
    test("should support contact relationships with multiple organizations", () => {
      ...
      expect(contactWithMultipleOrgs.organization_relationships).toHaveLength(
        2,
      );

      // Check primary organization
      const primaryRelationship =
        contactWithMultipleOrgs.organization_relationships?.find(
          (rel) => rel.is_primary,
        );
    ...
    test("should validate organization relationship fields", () => {
      ...
      expect(["High", "Medium", "Low", "Unknown"]).toContain(
        relationshipFields.purchase_influence,
      );
      expect([
        "Decision Maker",
        "Recommender",
        ...
        "End User",
        "Gatekeeper",
      ]).toContain(relationshipFields.decision_authority);
    });
  });
  ...
  describe("Search and Filter Opportunities", () => {
    test("should support enhanced filtering options", () => {
      ...
      expect(opportunity.probability).toBeGreaterThanOrEqual(
        filterCriteria.probability_min,
      );
      expect(opportunity.probability).toBeLessThanOrEqual(
        filterCriteria.probability_max,
      );
  ...
  describe("Backward Compatibility", () => {
    ...
    test("should validate data transformation compatibility", () => {
      ...
      expect(opportunityData.customer_organization_id).toBe(
        dealData.organization_id,
      );
  ...
  describe("Data Migration Verification", () => {
    test("should validate data preservation after migration", () => {
      ...
      expect(postMigrationOpportunity.customer_organization_id).toBe(
        preMigrationDeal.organization_id,
      );
      expect(postMigrationOpportunity.contact_ids).toEqual(
        preMigrationDeal.contact_ids,
      );
      ...
      expect(postMigrationOpportunity.description).toBe(
        preMigrationDeal.description,
      );
      expect(postMigrationOpportunity.amount).toBe(preMigrationDeal.amount);
      expect(postMigrationOpportunity.created_at).toBe(
        preMigrationDeal.created_at,
      );
