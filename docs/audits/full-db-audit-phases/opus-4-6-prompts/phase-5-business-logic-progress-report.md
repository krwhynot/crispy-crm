...
| 4 | Unified timeline (activities + tasks + notes) | VERIFIED | [Confidence: 95%] | `entity_timeline` view unions `activities` with `contact_notes`, `organization_notes`, and `opportunity_notes` (migration `20260210043844`). `UnifiedTimeline.tsx:86` fetches this resource. `TimelineEntry.tsx` supports `entry_type: "activity" | "task" | "note"` |
