...
### Violations Found (P1-3)

| Form | Issue |
|------|-------|
| ContactEdit | Missing `key={record.id}` |
| OrganizationEdit | Uses `defaultValues={record}` instead of schema.parse(record) |
| TaskEdit | Uses `Edit` + `SimpleForm` instead of `EditBase` + `Form` |
