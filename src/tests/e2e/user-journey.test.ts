...
const generateSyntheticData = () => {
  ...
  return {
    ...
    task: {
      ...
      status: "pending",
      due_date: faker.date
        .future({ refDate: new Date(), years: 0.083 })
        .toISOString(), // ~30 days
...
describe("E2E User Journey Tests", () => {
      ...
      const salesStages = [
        {
          stage: "qualification",
          probability: 25,
          requiredActivities: ["initial_call"],
        },
        {
          stage: "proposal",
          probability: 50,
          requiredActivities: ["needs_analysis", "proposal_sent"],
        },
        {
          stage: "negotiation",
          probability: 75,
          requiredActivities: ["proposal_review", "pricing_discussion"],
        },
        {
          stage: "closed-won",
          probability: 100,
  ...
  describe("Multi-Contact Sales Process", () => {
    test("complex sales process with multiple stakeholders", async () => {
      ...
      if (stakeholderTasks && stakeholderTasks.length >= 4) {
        ...
        console.warn(
          `⚠️ Expected 4 stakeholder tasks, got ${stakeholderTasks?.length || 0}`,
        );
      }
    });
  });
    ...
    test("maintain referential integrity throughout user journey", async () => {
      ...
      if (deleteError) {
        ...
        expect([null, undefined]).toContain(
          orphanedOpp?.customer_organization_id,
        );
      }
  ...
  describe("Performance and Scalability", () => {
    test("handle bulk operations efficiently", async () => {
      ...
      console.log(
        `✓ Bulk operation completed in ${duration}ms for ${batchSize * 2} records`,
      );
    });
  });
  ...
  describe("Error Handling and Recovery", () => {
    test("handle invalid data gracefully", async () => {
      ...
      if (testOpp) {
        ...
        console.warn(
          "⚠️ Opportunity created with inconsistent stage/probability combination",
        );
      }
    });
  });
