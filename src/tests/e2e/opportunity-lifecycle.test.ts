...
const generateOpportunityData = (overrides = {}) => {
  ...
  return {
    name:
      faker.company.name() + " " + faker.commerce.product() + " Opportunity",
...
describe("E2E Opportunity Lifecycle Tests", () => {
  ...
  describe("Sales Pipeline Progression", () => {
    test("systematic progression through all pipeline stages", async () => {
      ...
      for (const targetStage of progressionStages) {
        ...
        for (const activity of stageConfig.activities) {
          const { data: task } = await supabase
            ...
            .insert({
              ...
              type: activity.includes("call")
                ? "call"
                : activity.includes("meeting") || activity.includes("demo")
                  ? "meeting"
                  : activity.includes("email") || activity.includes("proposal")
                    ? "email"
                    : "follow_up",
        ...
        if (
          ["proposal", "negotiation", "closed-won", "closed-lost"].includes(
            targetStage,
          )
        ) {
          updateData.expected_close_date = new Date(
        ...
        console.log(
          `✓ Opportunity ${opportunity.id} progressed to ${targetStage} (${stageConfig.probability}% probability)`,
        );
      }
      ...
      if (activities && activities.length > 0) {
        expect(activities.every((task) => task.status === "completed")).toBe(
          true,
        );
        console.log(
          `✓ Created ${activities.length} activities for opportunity lifecycle`,
        );
      } else {
        console.warn(
          "⚠️ No activities created - may be due to schema constraints",
        );
      }
    });
  ...
  describe("Opportunity Line Items and Calculations", () => {
    test("manage opportunity line items with complex pricing", async () => {
      ...
      if (itemsError) {
        console.warn(
          "Line items creation error (table may not exist):",
          itemsError.message,
        );
      ...
      const expectedTotal = expectedCalculations.reduce(
      ...
      const categories = Array.from(
        new Set(itemsWithTotals.map((item) => item.category)),
      );
    ...
    test("handle line item modifications and recalculations", async () => {
      ...
      if (itemError) {
        console.warn(
          "Line items test skipped (table may not exist):",
          itemError.message,
        );
        return;
      }
  ...
  describe("Advanced Pipeline Analytics", () => {
    test("track stage duration and conversion metrics", async () => {
      ...
      if (progressionNotes && progressionNotes.length >= 3) {
        ...
        console.warn(
          `⚠️ Expected 3 stage duration notes, got ${progressionNotes?.length || 0}`,
        );
      }
      ...
      const probabilityIncrease =
        stageProgression[stageProgression.length - 1].probability - 10; // From initial 10%

      console.log(
        `Opportunity velocity: ${probabilityIncrease}% probability increase over ${totalDays} days`,
      );
      console.log(
        `Average days per stage: ${totalDays / stageProgression.length}`,
      );
    });
    ...
    test("forecast analysis based on pipeline health", async () => {
      ...
      // Group by stage
      const stageMetrics = pipelineAnalysis.reduce(
        (acc, opp) => {
          if (!acc[opp.stage]) {
      ...
      for (const [stage, metrics] of Object.entries(stageMetrics)) {
        console.log(
          `  ${stage}: ${metrics.count} opps, $${metrics.totalValue.toLocaleString()} total, $${metrics.weightedValue.toLocaleString()} weighted`,
        );
      }

      console.log(
        `Total Pipeline Weighted Value: $${weightedValue.toLocaleString()}`,
      );
  ...
  describe("Business Rule Validation", () => {
    test("enforce stage progression rules", async () => {
      ...
      if (backwardResult && backwardResult.stage === "lead") {
        console.warn(
          "⚠️ Backward stage progression allowed - consider adding business rules",
        );
      }
    ...
    test("validate required fields per stage", async () => {
      ...
      if (!missingAmountError) {
        console.warn(
          "⚠️ Proposal stage allows null amount - consider field validation",
        );
      }
      ...
      if (!missingDateError) {
        console.warn(
          "⚠️ Negotiation stage allows null close date - consider validation",
        );
      }
