...
describe("StageBadgeWithHealth", () => {
  ...
  it("applies success border for active health", () => {
    const { container } = renderWithAdminContext(<StageBadgeWithHealth stage="qualified" health="active" />);
    const badge = container.querySelector(".border-success");
    expect(badge).toBeInTheDocument();
  });

  it("applies warning border for cooling health", () => {
    const { container } = renderWithAdminContext(<StageBadgeWithHealth stage="qualified" health="cooling" />);
    const badge = container.querySelector(".border-warning");
    expect(badge).toBeInTheDocument();
  });

  it("applies destructive border for at_risk health", () => {
    const { container } = renderWithAdminContext(<StageBadgeWithHealth stage="qualified" health="at_risk" />);
