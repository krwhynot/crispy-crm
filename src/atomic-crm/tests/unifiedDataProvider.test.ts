      ...
      const commonErrors = [
        {
          ...
          suggestion: 'The field "nb_tasks" does not exist in contacts_summary. Available task-related queries should use the tasks table directly.'
        },
        {
          error: 'column contacts.last_activity does not exist',
          suggestion: 'The field "last_activity" does not exist. Use the activities table with a join or the contacts_summary view which has "last_seen".'
        },
        {
          error: 'relation "public.deals" does not exist',
          suggestion: 'The table "deals" does not exist. The system uses "opportunities" instead of deals.'
        }
      ];

      const getSuggestion = (errorMessage: string): string => {
        const error = commonErrors.find(e => errorMessage.includes(e.error.split(' ').slice(1).join(' ')));
        return error?.suggestion || 'Check the database schema for valid fields and tables.';
      };

      // Test suggestions
      expect(getSuggestion('column contacts_summary.nb_tasks does not exist'))
        .toContain('tasks table directly');

      expect(getSuggestion('column contacts.last_activity does not exist'))
        .toContain('activities table');

      expect(getSuggestion('relation "public.deals" does not exist'))
