...
describe('Data Provider Error Handling', () => {
  ...
  describe('Error Recovery Strategies', () => {
    it('should provide fallback when field does not exist', async () => {
      ...
      const result = safeQuery('contacts_summary',
        ['id', 'name', 'nb_tasks', 'last_seen'],
        { nb_tasks: { gt: 0 }, last_seen: { not: null } }
      );
  ...
  describe('Schema Validation at Runtime', () => {
    it('should validate schema before making requests', () => {
      ...
      expect(validation.errors).toContain("Field 'nb_tasks' does not exist in table 'contacts_summary'");
      expect(validation.errors).toContain("Filter field 'nb_tasks' does not exist in table 'contacts_summary'");
    });
  });
});
...
function getValidFields(table: string): string[] {
  const schemas: Record<string, string[]> = {
    'contacts_summary': [
      ...
      'last_seen', 'first_seen', 'tags', 'organization_ids'
    ],
    'contacts': [
      'id', 'first_name', 'last_name', 'email_addresses', 'phone_numbers',
