-- Fix get_activity_log RPC: Remove deleted_at checks for tables without soft delete
-- Engineering Constitution: FAIL FAST - remove checks for non-existent columns
-- Only organizations has deleted_at column, other tables don't support soft delete
...
BEGIN
  RETURN (
    WITH activity_events AS (
      -- 1. Organization created events (has deleted_at)
      SELECT
      ...
      -- 2. Contact created events (no deleted_at)
      SELECT
      ...
      -- 3. Contact note created events (no deleted_at)
      SELECT
      ...
      -- 4. Opportunity created events (no deleted_at)
      SELECT
      ...
      -- 5. Opportunity note created events (no deleted_at)
      SELECT
...
COMMENT ON FUNCTION get_activity_log IS 'Optimized activity log query - consolidates 5 queries into 1 server-side UNION ALL. Returns JSON array of activity events sorted by date DESC. Fixed to only check deleted_at on organizations table (the only table with soft delete support).';
