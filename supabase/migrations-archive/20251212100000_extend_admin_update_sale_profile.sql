-- Migration: Extend admin_update_sale to accept profile fields
-- Purpose: Allow non-admin users to edit their own profile (first_name, last_name, email, phone)
-- Previous version: 5 parameters (role, disabled, avatar, deleted_at)
-- New version: 9 parameters (+ first_name, last_name, email, phone)

-- Create the 9-parameter version of admin_update_sale
...
COMMENT ON FUNCTION admin_update_sale(UUID, user_role, BOOLEAN, TEXT, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT) IS
  'SECURITY DEFINER with authorization: Only admins can modify role/disabled/deleted_at. Non-admins can update their own profile fields (first_name, last_name, email, phone) and avatar. 9-parameter version supports profile editing.';

-- Grant execute to authenticated role
GRANT EXECUTE ON FUNCTION admin_update_sale(UUID, user_role, BOOLEAN, TEXT, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT) TO authenticated;
