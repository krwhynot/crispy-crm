-- Migration: Add search_tsv to organizations_summary view
-- Purpose: Enable PostgreSQL full-text search on organization list queries
-- Impact: View recreation (grants must be restored)
--
-- This is required for FTS migration - without search_tsv exposed on the view,
-- FTS queries against organizations_summary will fail.
--
-- Note: contacts_summary and opportunities_summary already include search_tsv.

-- Drop and recreate the view with search_tsv
...
SELECT
  ...
  o.email,
  o.linkedin_url,
  o.search_tsv,  -- FTS: Expose tsvector for full-text search queries
...
COMMENT ON VIEW organizations_summary IS
  'Organization list with hierarchy, rollup metrics, segment_name, org_scope, search_tsv, and audit columns. '
