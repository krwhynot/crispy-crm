-- Fix array parsing in sync_opportunity_with_products
-- The issue: JSONB arrays need proper conversion to PostgreSQL arrays
-- Using ->> on an array returns a text string like "[1644]" which fails to cast
-- Solution: Use JSONB array functions to extract and convert elements
...
-- Update comment
COMMENT ON FUNCTION public.sync_opportunity_with_products IS 'Atomically synchronize opportunity and its product associations (with proper JSONB array conversion)';
