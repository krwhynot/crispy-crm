-- Fix sync_opportunity_with_products RPC to use soft delete instead of hard DELETE
-- Closes critical issue #4 from audit 2026-01-26
--
-- Changes:
-- 1. Replace hard DELETE with UPDATE deleted_at = NOW()
-- 2. Modify INSERT to restore soft-deleted records on conflict
...
BEGIN
  ...
  -- Soft delete removed product associations (was: hard DELETE)
...
-- Update comment to reflect soft delete behavior
COMMENT ON FUNCTION public.sync_opportunity_with_products IS
  'Atomically synchronize opportunity and its product associations. Uses soft delete for removed products.';
