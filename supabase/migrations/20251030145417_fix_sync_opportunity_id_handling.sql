-- Fix sync_opportunity_with_products to handle new opportunities correctly
-- Issue: INSERT was trying to include id column with NULL value, violating NOT NULL constraint
-- Solution: Separate INSERT path for new opportunities (omit id, let database generate)
...
COMMENT ON FUNCTION public.sync_opportunity_with_products IS 'Atomically synchronize opportunity and its product associations (with comprehensive backend validation enforcing business rules). Fixed to properly handle new opportunities by omitting id in INSERT.';
