/**
 * Fix complete_task_with_followup RPC Function - created_by Lookup
 *
 * Problem: auth.uid() is UUID, cannot be cast to BIGINT for created_by field
 * Solution: Look up sales.id from auth.uid() using subquery
 *
 * Error fixed: "cannot cast type uuid to bigint"
 */
...
DECLARE
  ...
  v_sales_id BIGINT;
BEGIN
  -- Look up sales_id for the current user
  ...
  IF v_task.opportunity_id IS NOT NULL THEN
    ...
    INSERT INTO activities (
      ...
      p_task_id,
      v_sales_id  -- Fixed: use v_sales_id instead of auth.uid()::BIGINT
